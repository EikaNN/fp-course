...
filter p = foldRight (\a acc -> if p a then a :. acc else acc) Nil
...
flatten = foldRight (\a acc -> a ++ acc)  Nil
...
find _ Nil = Empty
find p (a :. as) = if p a then Full a else find p as
